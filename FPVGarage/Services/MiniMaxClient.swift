...
final class MiniMaxClient: LLMClientProtocol {
    ...
    init(
        ...
        baseURL: String = LLMProvider.minimax.baseURL
    ) {
    ...
    func send(request: LLMRequest) async throws -> AsyncStream<LLMChunk> {
        guard let apiKey = apiKeyStore.load(for: LLMProvider.minimax.keychainKey) else {
